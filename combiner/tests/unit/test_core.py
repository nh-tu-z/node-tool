...
class TestParseEncodeHeader:
    ...
    def test_parse_valid_header_with_encoding(self):
        """Test parsing a valid header with encoding (legacy format, no sha256)."""
        line = "#begin#src/main.py#42#text#/begin#\n"
        
        result = _parse_encode_header(line)
        
        assert result == ("src/main.py", 42, "text", None)
    
    def test_parse_valid_header_base64(self):
        """Test parsing a header with base64 encoding (legacy format, no sha256)."""
        line = "#begin#images/logo.png#0#base64#/begin#\n"
        
        result = _parse_encode_header(line)
        
        assert result == ("images/logo.png", 0, "base64", None)
    ...
    def test_parse_legacy_format(self):
        ...
        assert result == ("src/test.js", 15, "text", None)
    ...
    def test_parse_header_with_zero_lines(self):
        """Test parsing header for empty or binary file (legacy format, no sha256)."""
        line = "#begin#empty.txt#0#text#/begin#\n"
        
        result = _parse_encode_header(line)
        
        assert result == ("empty.txt", 0, "text", None)


    def test_parse_header_with_sha256(self):
        """Test parsing a header with SHA-256 hash (current format)."""
        sha256 = 'a' * 64  # Fake 64-char hex digest
        line = f"#begin#src/main.py#42#text#{sha256}#/begin#\n"
        
        result = _parse_encode_header(line)
        
        assert result == ("src/main.py", 42, "text", sha256)


    def test_parse_header_with_sha256_base64(self):
        """Test parsing a base64 header with SHA-256 hash (current format)."""
        sha256 = 'b' * 64
        line = f"#begin#assets/logo.png#0#base64#{sha256}#/begin#\n"
        
        result = _parse_encode_header(line)
        
        assert result == ("assets/logo.png", 0, "base64", sha256)
