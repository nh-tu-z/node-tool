...
class TestCombineSpreadRoundtrip:
    ...
    def test_roundtrip_with_exclusions(self, temp_source_dir, tmp_path, create_test_files, mock_uuid):
        ...
        # Only non-excluded files should be spread
        assert (spread_dir / "src" / "app.py").exists()
        assert (spread_dir / "test" / "test.py").exists()
        assert not (spread_dir / "build" / "output.js").exists()
